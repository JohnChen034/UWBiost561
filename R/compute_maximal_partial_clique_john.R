...
#' Safe function of master function
#'
#' Same functionality of master fucntion, only add a error catch for those
#' implementation who has error generate a matrix.
#' @param adj_mat a symmetric \code{matrix} of 0/1 with 1’s on the diagonal,
#’   no row- or column-names, and size between 5 and 50.
#' @param alpha a single \code{numeric} in \[0.5, 1\].
#' @param number an integer from 1 to 15 selecting the implementation.
#' @param time_limit maximum seconds before timing out.
#'
#' @return a list with `clique_idx`, `edge_density` (which are from the implementation itself)
#' and `status` and `valid` (which are added by this function)
#' @export
compute_maximal_partial_clique_master_safe <- function(adj_mat,
                                                       alpha,
                                                       number,
                                                       time_limit = 30) {
  result <- tryCatch({
  ...
  if (!all(c("clique_idx", "edge_density", "status", "valid") %in% names(result))) {
    result <- list(
      clique_idx   = NA_integer_,
      edge_density = NA_real_,
      status       = "error",
